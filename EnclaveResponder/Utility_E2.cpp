                                ...
                                , ms_in_msg_exchange_t* ms)
{
    ...
    buff = ms->inparam_buff;
    len = ms->inparam_buff_len;
    if(len != sizeof(worker_input_t))
        return ATTESTATION_ERROR;

    memcpy(inp_secret_data, buff, sizeof(worker_input_t));
...
uint32_t marshal_message_exchange_response(char** resp_buffer, size_t* resp_length, worker_output_t secret_response)
{
    ...
    if(!resp_length)
        return INVALID_PARAMETER_ERROR;    
    secret_response_len = sizeof(worker_output_t)+24;
...
float computeEuclideanDistance(float *point1, float *point2, int dimension) {
    float sum = 0.0;
...
/*int main() {
    centroid* matrix = create_centroid(MATRIX_SIZE, MATRIX_SIZE);
    set_centroid_value(matrix, 0, 0, 1.0);
    set_centroid_value(matrix, 0, 1, 2.0);
    set_centroid_value(matrix, 1, 0, 3.0);
    set_centroid_value(matrix, 1, 1, 4.0);
    double value = get_centroid_value(matrix, 1, 1);
    destroy_centroid(matrix);
    return 0;
}*/